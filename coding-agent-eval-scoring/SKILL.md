...
성적서에는 누를 수 있는 판정 174개와 근거 195개가 붙어 있다. 근거 수집은 항목 묶음으로
나눠 병렬로 돌려 236개를 모았고 그중 195개를 실었다. 조사와 검증 과정에서 성적서 자체의
사실 오류를 **열여덟 건** 찾아 고쳤다. 대부분 정적으로 훑어서는 보이지 않는 것들이었다.
...
근거를 붙여도 읽는 사람은 여전히 "그래서 원래 이슈에는 뭐라고 적혀 있었나"를 묻는다.
**본문의 이슈·PR 번호를 전부 링크로 만든다.** 한 회차에서 274곳이 나왔다.

맨 앞에 회차별 대응표를 둔다. 같은 프롬프트로 이슈를 둘 내고 하나는 대상 모델이,
하나는 대조군이 받았다는 구조가 표 하나로 보인다.

| 회차 | 과제 | 레벨 | 대상 이슈 | 대상 PR | 대조 이슈 | 대조 PR |
|---|---|---|---|---|---|---|

**인용한 코드 안의 번호는 링크로 만들지 않는다.** 원본에 링크를 넣으면 그 코드가
실제와 달라진다. `References #230` 이 PR 본문에 적힌 그대로여야 검증이 성립한다.
링크는 본문에만 걸고 `<pre>` 안은 손대지 않는다.
...
수집 담당은 자기 결과를 계속 개선하려 한다. 좋은 태도지만 **뒤 단계에서는 재작업을
유발한다.** 한 회차에서 한 담당의 결과가 세 번 갱신됐고(근거 51개 → 53개 → 78개)
그때마다 표현 담당에게 "다시 읽어라"를 보내야 했다.

앞 단계가 끝나면 **사본을 떠서 고정하고 그 사본만 읽게 한다.** 수집 담당에게는
"더 고칠 것이 보이면 파일을 고치지 말고 메시지로 알려라"고 전한다. 판단은 조율하는
쪽이 한다.
...
- **PR 본문만 보고 "근거 없음"이라 결론했다.** 사람이 나중에 값을 고쳤기 때문이다.
  `userContentEdits` 로 편집 이력을 보면 원본이 남아 있다. 실제로 한 PR의 성능 지표 표에서
  원본이 `모델명 Claude · 12분 · 입력 52,000/출력 12,000` 이었고 13분 뒤 사람이
  `Qwen3.8` 과 `(추정치)` 로 고친 기록이 나왔다. 이 확인이 판정을 되살렸다.
...
성적서를 스크립트로 만들다 보면 같은 함정에 반복해서 빠진다. 한 회차에서 겪은 것들이다.

### 백슬래시가 세 겹으로 해석된다

문자열이 셸, 파이썬, 정규식 치환을 차례로 지나면서 각 단계가 백슬래시를 먹는다.
같은 원인으로 세 번 깨졌다.

| 어디서 | 무엇을 썼나 | 무엇이 됐나 |
|---|---|---|
| 파이썬 문자열 | `content:"\2192"` (CSS 화살표) | `\21` 을 8진수로 읽어 제어문자 + `"92"` |
| `re.sub` 치환 | 본문에 `\#226` 이 있는 상태로 치환 | `bad escape \m` 으로 실패 |
| 셸 heredoc | 따옴표 없는 heredoc 안의 백틱 | 셸이 명령 치환으로 실행 |

**이스케이프를 피할 수 있으면 피한다.** CSS 화살표는 `content:"→"` 처럼 문자를 직접
넣는다. `re.sub` 의 치환은 `lambda _: 문자열` 로 리터럴 처리한다. heredoc 은 항상
`<<'EOF'` 로 따옴표를 붙인다.

### 뷰포트가 아니라 컨테이너 폭으로 판단한다

성적서를 아티팩트로 배포하면 프레임 안에서 렌더링된다. 게다가 대상과 대조군을
좌우로 나누면 실제 칸은 화면의 절반이고, 그 안에서 원본과 설명을 다시 나누면 4분의 1이다.
**뷰포트 기준 미디어쿼리는 이 상황을 보지 못한다.** 화면이 아무리 넓어도 칸은 좁다.

컨테이너 쿼리를 쓴다. 근거 상자에 `container-type:inline-size` 를 주고
`@container (min-width:640px)` 로 판단하면 실제 폭에 맞게 접힌다.

### 증상이 몰린 곳을 먼저 본다

붉은 설명 글씨가 한 글자씩 세로로 늘어졌다. 폭 문제로 짐작해 두 번 고쳤지만 낫지 않았다.
원인은 상속된 그리드였다.

```css
.cmp li { display:grid; grid-template-columns:14px 1fr; }
```

판정 목록의 표시 칸(`✓`/`✗`)을 위한 규칙인데 화살표 목록의 `li` 에도 걸렸다.
화살표 `li` 는 자식이 텍스트 하나뿐이라 14px 칸에 들어가 늘어졌다.

**"다른 데는 멀쩡한데 이것만 그렇다"가 범위를 좁히는 단서다.** 폭 문제라면 옆 칸도
같이 좁아야 하는데 그렇지 않았다. 증상이 한 요소에만 몰리면 그 요소에만 걸리는 규칙을
찾아야 한다. 전역 규칙을 의심하기 전에 선택자 범위를 본다.

### 마크업은 스크립트로 만들고 구조를 검사한다

손으로 고치면 태그가 깨진다. 한 회차에서 두 번 깨뜨렸다. 정규식으로 블록을 갈아 끼울
때는 여는 태그와 닫는 태그 수를 매번 센다.

```
div dd dl dt pre span details summary ul li tr td p section table
```

빈 블록, 이스케이프되지 않은 원시 `<`, 중복 `id` 도 함께 본다. 생성기를 만들었으면
가짜 명세로 먼저 돌려 본다. 실제 명세로 처음 돌렸을 때 깨지는 것보다 낫다.
...
| 단계 사이에 입력을 동결하지 않음 | 앞 단계가 계속 개선되면 뒤 단계가 재작업된다 |
| 이슈·PR 번호를 텍스트로만 둠 | 읽는 사람이 원문을 확인할 길이 없다. 링크로 만들어라 |
| 인용한 코드 안까지 링크로 만듦 | 원본이 실제와 달라져 검증이 성립하지 않는다 |
| 화면 폭 기준으로 배치를 판단 | 프레임과 중첩 칸 때문에 실제 폭은 훨씬 좁다 |
| 증상이 한 곳에만 몰렸는데 전역 원인을 의심 | 그 요소에만 걸리는 규칙부터 찾아라 |
